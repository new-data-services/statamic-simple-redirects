...
@section('content')
    <div class="mb-6">
        <h1 class="text-3xl font-bold">{{ __('Create Redirect') }}</h1>
    </div>

    <form action="{{ cp_route('simple-redirects.store') }}" method="POST" class="publish-form card">
        @csrf

        <div class="form-group">
            <label class="font-bold text-base mb-sm">{{ __('Source URL') }} <i class="required">*</i></label>
            <input type="text" name="source" class="input-text" value="{{ old('source') }}" required>
        ...
        <div class="form-group">
            ...
            <input type="text" name="destination" class="input-text" value="{{ old('destination') }}" required>
        ...
        <div class="form-group">
            ...
            <select name="type" class="input-text" required>
                <option value="exact" {{ old('type') === 'exact' ? 'selected' : '' }}>{{ __('Exact Match') }}</option>
                <option value="regex" {{ old('type') === 'regex' ? 'selected' : '' }}>{{ __('Regular Expression') }}</option>
            </select>
            @error('type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="form-group">
            ...
            <select name="status_code" class="input-text" required>
                <option value="301" {{ old('status_code') === '301' ? 'selected' : '' }}>301 - {{ __('Permanent') }}</option>
                <option value="302" {{ old('status_code') === '302' ? 'selected' : '' }}>302 - {{ __('Temporary') }}</option>
                <option value="410" {{ old('status_code') === '410' ? 'selected' : '' }}>410 - {{ __('Gone') }}</option>
            </select>
            @error('status_code')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
